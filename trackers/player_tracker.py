...
class PlayerTracker:
    ...
    def detect_frame(self,frame):
        results=self.model.track(frame ,persist=True)[0] #persist is toi remember the pervious tracking
        ...
        for box in results.boxes:
            ...
            objext_cls_name=id_name_dict[objext_cls_id]
            if objext_cls_name=="person":
    ...
    def draw_bboxes(self,video_frame,player_detection):
        ...
        for frame,player_dict in zip(video_frame,player_detection):
            for track_id, bbox in player_dict.items():
                x1,y1, x2,y2 = bbox
                cv2.putText(frame,f"player_id : {track_id}",(int(bbox[0]),int(bbox[1]-10)),cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,225),2)
